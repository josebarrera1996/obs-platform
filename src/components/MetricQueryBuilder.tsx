...
export function MetricQueryBuilder({
  ...
  useEffect(() => {
    ...
    } else if (defaultDimensions?.ClusterName) {
      // Grafana-style default: cluster only when matchExact is off
      setDimensions({ ClusterName: defaultDimensions.ClusterName });
  ...
  // When metric changes, suggest dimensions (only if none configured yet)
  useEffect(() => {
    ...
    if (!metric || metric.dimensionSets.length === 0) return;
    ...
    if (matchExact) {
      ...
      const cluster = firstSet.find((d) => d.Name === "ClusterName");
      if (cluster) {
  ...
  }, [metricName, availableMetrics, matchExact]);
  ...
  // ── Available dimension names based on selected metric ──
      ...
      <DialogContent className="min-w-[680px] max-w-[92vw] max-h-[92vh] overflow-y-auto flex flex-col gap-0 p-0">
        ...
        <div className="px-6 py-5 space-y-6 flex-1">
                  ...
                  <SelectContent className="max-h-64">
                    {filteredMetrics.length === 0 ? (
                      ...
                      filteredMetrics.map((m) => (
                        <SelectItem key={m.metricName} value={m.metricName} className="text-sm">
                          {m.metricName}
                        </SelectItem>
                      ))
                    )}
            ...
            {dimensionKeys.length === 0 ? (
              ...
              <div className="space-y-2">
                {dimensionKeys.map((key) => {
                  const dimOpt = dimensionOptions.find((d) => d.name === key);
                  return (
                    <div key={key} className="flex items-center gap-2">
                      {/* Dimension name */}
                      {metricDimNames.length > 0 ? (
                        <Select
                          value={key}
          ...
          <div className="space-y-1.5">
            ...
            <Select value={String(period)} onValueChange={(v: string | null) => v && setPeriod(Number(v))}>
